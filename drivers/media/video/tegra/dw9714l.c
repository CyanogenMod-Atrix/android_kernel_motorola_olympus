...
#ifdef PWR_DIRECT_REGULATOR_CONTROL
	struct regulator *regulator;
#else
	struct dw9714l_platform_data *pdata;
#endif
...
static int dw9714l_open(struct inode *inode, struct file *file)
{
	...
	if (info->pdata && info->pdata->power_on)
		info->pdata->power_on(info->pdata->power_id);
#endif
	return 0;
}
...
int dw9714l_release(struct inode *inode, struct file *file)
{
	...
	if (info->pdata && info->pdata->power_off)
		info->pdata->power_off(info->pdata->power_id);
#endif
...
#ifdef PWR_DIRECT_REGULATORS_CONTROL
...
#ifndef PWR_DIRECT_REGULATORS_CONTROL
	if (info->pdata && info->pdata->power_off)
		info->pdata->power_off(info->pdata->power_id);
#endif
	return 0;
