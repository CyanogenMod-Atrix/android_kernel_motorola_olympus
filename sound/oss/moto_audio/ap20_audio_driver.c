...
static long audio_ioctl(struct file *file,
			unsigned int cmd, unsigned long arg)
{
	...
	case SNDCTL_DSP_STEREO:
	{
		...
		if (minor == state.dev_dsp) {
			...
			if (file->f_mode & FMODE_WRITE) {
				...
				capture_mode = val;
				TRY(audio_stop_ssi(file->f_dentry->d_inode, file))
				TRY(audio_configure_ssi(file->f_dentry->d_inode, file))
			}
		}
		break;
	}
	...
	case SNDCTL_DSP_RESET:
	{
		...
		TRY(omap2_mcbsp_set_xrst(ssi, OMAP_MCBSP_XRST_DISABLE))
#endif // MCBSP_WRAPPER
		audio_buffer_reset(str, file->f_dentry->d_inode);
		break;
	}
